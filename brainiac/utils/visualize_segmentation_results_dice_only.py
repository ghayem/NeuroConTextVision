...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate side-by-side surface binarized segmentations with Dice score."
    )
    parser.add_argument(
        "--input_dir",
        type=str,
    ...
    for gt_path in ground_truth_paths:
        ...
        # --- Plot GT surface ---
        result_gt = plotting.plot_img_on_surf(gt_norm_img, title="Ground Truth", **plot_parameters)
        fig_gt = result_gt[0] if isinstance(result_gt, tuple) else result_gt

        # --- Plot Seg surface ---
        result_seg = plotting.plot_img_on_surf(seg_norm_img, title="Segmented", **plot_parameters)
        fig_seg = result_seg[0] if isinstance(result_seg, tuple) else result_seg

        # --- Render both figures to numpy arrays ---
        fig_gt.canvas.draw()
        fig_seg.canvas.draw()
        ...
        # --- Pad heights if they differ, then combine side-by-side ---
